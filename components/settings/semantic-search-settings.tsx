...
function ProgressDisplay({
    ...
    if (phase === "model") {
        ...
        const dtypeStr = isDownloading && modelLoadingDtype
        ...
        const speedStr =
            isDownloading && modelLoadingSpeedBytesPerSec > 0
                ...
                : null;
        const bytesLine = isDownloading && hasAggBytes
...
async function getModelCacheSize(): Promise<number> {
    ...
    try {
        ...
        const modelKeys = keys.filter((req) => req.url.includes(EMBEDDING_MODEL_ID));
        let total = 0;
...
function ModelCacheManager({ fileProgress }: { fileProgress: Record<string, { loaded: number; total: number }> }) {
...
export function SemanticSearchSettings() {
    ...
    const { updateSettings, semanticDtype, semanticAutoIndexing, semanticSearchEnabled } =
        useGeneralStore();
    ...
    let indexEta = "";
    if (processedCount > 0 && remaining > 0 && indexStartTimeRef.current !== null) {
    ...
    const isFullyIndexed =
        (embeddingStats?.indexedBookmarks ?? 0) === (embeddingStats?.totalBookmarks ?? 0) &&
