    ...
    if (!user) {
        return (
            <div className={cn("flex items-center gap-2 px-2 py-2", className)}>
                <div className={cn("rounded-full bg-sidebar-accent/20 animate-pulse", avatarClassName)} />
                <div className="flex-1 space-y-1 min-w-0">
                    <div className="h-3 w-20 rounded bg-sidebar-accent/20 animate-pulse" />
                    {showEmail && <div className="h-2 w-32 rounded bg-sidebar-accent/20 animate-pulse" />}
                </div>
            </div>
        );
    }
    ...
    return (
        <div className={cn("flex items-center gap-2 px-2", className,
            ...
            <div className="flex-1 min-w-0 text-left">
                <p className={cn("truncate text-sm font-medium leading-tight text-sidebar-foreground")}>
                    {user.name}
                </p>
                {showEmail && (
